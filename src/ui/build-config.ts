 ...
 *
 * Translates RazielTeams's CreateConfigValues into the adapterConfig
...
import type { CreateConfigValues } from "@raziel-teams/adapter-utils";
...
/**
 * Build a Hermes Agent adapter config from the RazielTeams UI form values.
 */
export function buildHermesConfig(
  ...
  // NOTE: Provider is NOT set here because the RazielTeams UI form
  ...
  // Execution limits — let the user configure these from the RazielTeams UI.
  ...
  // Heartbeat config is handled by RazielTeams itself
